...
export default function LoginPage() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!email) {
            setError((prev) => ({ ...prev, email: loginErrors.email.empty }));
            return;
        } else if (!/\S+@\S+\.\S+/.test(email)) {
            setError((prev) => ({ ...prev, email: loginErrors.email.invalid }));
            return;
        }

        if (!password) {
            setError((prev) => ({ ...prev, password: loginErrors.password.empty }));
            return;
        } else if (password.length < 6) {
            setError((prev) => ({ ...prev, password: loginErrors.password.invalid }));
            return;
        }

        setSuccess(loginErrors.form.success);
    };
